...
§b位 置:  §6坐标§e ${warp.post[0]} ${warp.post[1]} ${warp.post[2]} 
    ...
    */
    ...
    WriteDateWarp: function (filePath, fileName, content) {
        ...
        FileOppeate.FileWrite(filePath, fileName, JSON.stringify(WarpList))
    },
    /** 写入 Player 类型文件
     * @param filePath 文件路径
     * @param fileName 文件名称
    ...
    */
    Delete: function (_pl, warp, filePath, fileName) {
        if (fileName == CONFIG.LIST) {
            ...
            return FileOppeate.FileWrite(filePath, fileName, JSON.stringify(filedate))
        } else {
            var filedate = FileOppeate.FileRead(filePath, fileName)
            for (p in filedate) {
                filelist = filedate[p]
